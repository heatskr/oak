...
class Router {
  ...
  _error_handler(req, res, error) {
    var msg;
    console.error (error);
    msg = process.env.NODE_ENV === "production" ? "" : error.toString ();
    res.status (500).end (msg);
  }

  _dispatch(request, response) {
    var entry, it, matches, next, route, stop;
    it = this._routes.entries ();
    stop = false;
    next = (error) => {
      if (error) {
    ...
    while (entry = it.next ()) {
      ...
      route = entry.value[1];
      if (route.method.test (request.method) && route.pattern.test (request.path)) {
        matches = route.pattern.exec (request.path);
